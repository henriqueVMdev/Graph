<template>
  <div class="h-[calc(100vh-3.5rem)] overflow-y-auto p-4 space-y-4">
    <div class="flex flex-wrap items-center gap-3">
      <h1 class="text-base font-semibold text-gray-100">Screener de Perps</h1>
      <span class="text-[10px] text-gray-600 font-mono">bybit · top por volume · atualiza 60s</span>
      <div class="flex-1" />
      ...
      <select v-model="fundingFilter" class="form-select !py-1 text-xs">
        ...
        <tbody>
          <tr v-for="(r, i) in rows" :key="r.symbol"
            ...
            <td class="px-3 py-1.5 text-right whitespace-nowrap">
              <button @click="terminal.addToWatchlist(r.base)" title="+ Watchlist"
                      class="text-gray-600 hover:text-accent-yellow text-xs px-1">👁</button>
              <button @click="toBacktest(r.base)" title="Backtest"
                      class="text-gray-600 hover:text-accent-yellow text-xs px-1">⚡</button>
            </td>
          </tr>
        </tbody>
