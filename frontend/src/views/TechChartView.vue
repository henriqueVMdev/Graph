<template>
  <div class="h-[calc(100dvh-3.5rem)] overflow-y-auto p-4 space-y-4">
    <div class="flex flex-wrap items-center gap-3">
      ...
      <span class="text-[11px] text-gray-500 font-mono">multi-ativo · cripto + tradicional no mesmo gráfico</span>
      <div class="flex-1" />
    ...
    <div v-if="tab === 'grafico'" class="card p-3 space-y-2">
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-[11px] text-gray-500 uppercase tracking-wider">Estudos</span>
      ...
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-[11px] text-gray-500 uppercase tracking-wider">Custom</span>
    ...
    <template v-if="tab === 'grafico' && singleData && !loading">
      ...
      <div class="card p-3">
        <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">Volume financeiro ($)</div>
        <div ref="volChart" style="min-height:130px;" class="w-full"></div>
      </div>
      <div v-for="(p, i) in singleData.panels" :key="p.name + i" class="card p-3">
        <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">{{ p.name }}</div>
    ...
    <template v-if="tab === 'comparar' && cmpData && !loading">
      <div class="card p-3">
        <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">
          Retornos acumulados (%) — {{ cmpData.n_common }} candles em comum
        </div>
        <div ref="cumChart" style="min-height:320px;" class="w-full"></div>
      </div>
      <div class="card p-3">
        <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">Matriz de correlação (retornos)</div>
    ...
    <template v-if="(tab === 'spread' || tab === 'contratos') && spreadData && !loading">
      ...
      <div class="card p-3">
        <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">Retornos acumulados (%)</div>
        <div ref="spCumChart" style="min-height:260px;" class="w-full"></div>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="card p-3">
          <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">Spread (A − B)</div>
          <div ref="spDiffChart" style="min-height:220px;" class="w-full"></div>
        </div>
        <div class="card p-3">
          <div class="text-[11px] text-gray-400 uppercase font-semibold px-1">Razão (A / B)</div>
