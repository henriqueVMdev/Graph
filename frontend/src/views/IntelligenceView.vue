<template><div class="h-[calc(100dvh-3.5rem)] overflow-y-auto p-4 space-y-4">
 ...
 <div class="card p-4 overflow-x-auto"><div class="flex items-center gap-3 mb-2"><div class="title !mb-0">Ranking do universo — onde estão os melhores setups</div>
  ...
  <table class="w-full text-xs font-mono whitespace-nowrap"><thead><tr class="text-gray-500 text-left"><th class="py-1">Ativo</th><th>Classe</th><th>Sinal</th><th class="text-right">Score</th><th class="text-right">Conf.</th><th class="text-right">Cobertura</th><th class="text-right">Fatores</th><th class="text-left pl-3">Divergências</th></tr></thead>
   <tbody><tr v-for="r in rank?.rows||[]" :key="r.symbol" @click="r.score!=null&&(symbol=r.symbol,load())"
    ...
    <td class="pl-3 text-[11px] text-amber-400/80 max-w-[24rem] truncate">{{r.error||(r.divergences||[]).join(' · ')||'—'}}</td>
   </tr></tbody></table>
 ...
 <div class="card p-4 overflow-x-auto" v-if="track"><div class="title">Auto-auditoria forward — o que os sinais gravados renderam de verdade</div>
  ...
  <details v-if="(track.signals||[]).length" class="mt-2"><summary class="text-[11px] text-gray-400 cursor-pointer uppercase">últimos sinais gravados ({{track.signals.length}})</summary>
   <table class="w-full text-[11px] font-mono mt-2"><thead><tr class="text-gray-500 text-left"><th>Data</th><th>Ativo</th><th>Sinal</th><th class="text-right">Score</th><th class="text-right">Preço no sinal</th><th class="text-right">7d depois</th><th class="text-right">30d depois</th></tr></thead>
    <tbody><tr v-for="s in track.signals" :key="s.symbol+s.date" class="border-t border-surface-600/40">
     ...
     <td :class="s.label==='COMPRA'?'text-green-400':s.label==='VENDA'?'text-red-400':'text-gray-400'">{{s.label}}</td>
 ...
 <div class="card p-4 overflow-x-auto"><div class="title">Validação histórica dos sinais</div><table class="w-full text-xs font-mono"><thead><tr class="text-gray-500"><th class="text-left">Sinal</th><th>Dias</th><th>Ret. 30d</th><th>Acerto 30d</th><th>Ret. 90d</th></tr></thead><tbody><tr v-for="(v,k) in d.validation" :key="k" class="border-t border-surface-600"><td class="py-2">{{k}}</td><td class="text-center">{{v.samples}}</td><td class="text-center">{{pct(v.return_30d_pct)}}</td><td class="text-center">{{pct(v.win_rate_30d_pct)}}</td><td class="text-center">{{pct(v.return_90d_pct)}}</td></tr></tbody></table></div>
