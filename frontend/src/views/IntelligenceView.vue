<template><div class="h-[calc(100dvh-3.5rem)] overflow-y-auto p-4 space-y-4">
 ...
 <div class="card p-4 overflow-x-auto"><div class="flex items-center gap-3 mb-2"><div class="title !mb-0">Ranking do universo — onde estão os melhores setups</div>
  ...
  <span v-else-if="rank?.ts" class="text-[10px] text-gray-500 font-mono">atualizado {{new Date(rank.ts).toLocaleTimeString('pt-BR')}}</span></div>
  ...
  <p class="text-[10px] text-gray-500 mt-2">score = soma dos votos dos fatores · clique numa linha para a análise completa · universo: cripto, commodities CFTC, índices, FX e ações-tema</p>
 </div>
 <div class="card p-4 overflow-x-auto" v-if="track"><div class="title">Auto-auditoria forward — o que os sinais gravados renderam de verdade</div>
  ...
  <details v-if="(track.signals||[]).length" class="mt-2"><summary class="text-[10px] text-gray-400 cursor-pointer uppercase">últimos sinais gravados ({{track.signals.length}})</summary>
  ...
  <p class="text-[10px] text-gray-500 mt-2">{{track.method}}</p>
 </div>
 <template v-if="d&&!loading"><div class="grid grid-cols-2 md:grid-cols-5 gap-2">
  ...
  <div class="metric-card"><span class="metric-label">Confiança histórica</span><span class="metric-value text-accent-yellow">{{d.signal.confidence}}%</span><small class="text-[8px] text-gray-500">cobertura {{d.signal.coverage_pct??'—'}}%</small></div>
...
<style scoped>.title{@apply text-[10px] text-gray-400 uppercase font-semibold mb-2}</style>
