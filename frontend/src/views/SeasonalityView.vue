...
async function load(){loading.value=true;error.value=null;try{d.value=(await getSeasonality(symbol.value.trim().toUpperCase())).data;await draw()}catch(e){error.value=e.response?.data?.error||e.message;d.value=null}finally{loading.value=false}}onMounted(load)
</script>
