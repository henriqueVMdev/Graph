  ...
  <div class="h-[calc(100dvh-3.5rem)] overflow-y-auto p-4 space-y-4">
    ...
    <template v-if="d&&!loading">
      <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
        ...
        <div class="metric-card"><span class="metric-label">Melhor mês</span><span class="metric-value text-accent-yellow">{{monthName(d.best_months[0]?.month)}} {{pct(d.best_months[0]?.avg_pct)}}</span></div>
        <div class="metric-card"><span class="metric-label">Pior mês</span><span class="metric-value text-accent-red-light">{{monthName(d.worst_months[0]?.month)}} {{pct(d.worst_months[0]?.avg_pct)}}</span></div>
      ...
      <div class="card p-4 overflow-x-auto"><div class="title">Heatmap de performance mensal (%)</div>
        <table class="w-full text-[11px] font-mono"><thead><tr><th scope="col" class="text-left">Ano</th><th scope="col" v-for="m in 12" :key="m">{{monthName(m).slice(0,3)}}</th></tr></thead>
...
const layout=h=>({template:'plotly_dark',paper_bgcolor: TEMA.fundoPapel,plot_bgcolor: TEMA.fundoPlot,font:{color:'#aaa',size:10},height:h,margin:{t:10,r:45,b:35,l:45},xaxis:{gridcolor: TEMA.grade},yaxis:{gridcolor: TEMA.grade,ticksuffix:'%'},legend:{orientation:'h',y:1.12}})
...
async function draw(){const P=await getPlotly();await nextTick();const c={'20':'#f5c518','15':'#4dabf7','10':'#69db7c','5':'#f59f00','2':'#f74870'}
