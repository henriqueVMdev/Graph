...
async function render() {
  ...
  const traces = [{
    ...
    increasing: { line: { color: DIRECAO.alta } }, decreasing: { line: { color: DIRECAO.baixa } },
    yaxis: 'y',
  }]
  ...
  if (ov.volume && c.volume) {
    traces.push({
      ...
      marker: { color: c.close.map((cl, i) => cl >= c.open[i] ? rgbaAlta(0.3) : rgbaBaixa(0.3)) },
    })
  }
  if (ov.indicators && ind.ma) traces.push({ type: 'scatter', mode: 'lines', name: 'MA', x: c.dates, y: ind.ma, line: { color: DIRECAO.mediaRapida, width: 1.5 } })
  if (ov.indicators && ind.ma_slow) traces.push({ type: 'scatter', mode: 'lines', name: 'MM Lenta', x: c.dates, y: ind.ma_slow, line: { color: DIRECAO.mediaLenta, width: 1.5 } })
  ...
  if (ov.markers) {
    ...
    if (longX.length) traces.push({ type: 'scatter', mode: 'markers', name: 'Entrada Long', x: longX, y: longY, marker: { symbol: 'triangle-up', size: 10, color: DIRECAO.alta, line: { color: DIRECAO.altaBorda, width: 1 } } })
    if (shortX.length) traces.push({ type: 'scatter', mode: 'markers', name: 'Entrada Short', x: shortX, y: shortY, marker: { symbol: 'triangle-down', size: 10, color: DIRECAO.baixa, line: { color: DIRECAO.baixaBorda, width: 1 } } })
  ...
  if (ov.stops) {
    for (const t of trades) {
      if (t.entry_ts && t.exit_ts && t.stop_price != null && t.target_price != null) {
        ...
        shapes.push({ type: 'line', xref: 'x', yref: 'y', x0, x1, y0: t.stop_price, y1: t.stop_price, line: { color: rgbaBaixa(0.7), width: 1, dash: 'dot' } })
        shapes.push({ type: 'line', xref: 'x', yref: 'y', x0, x1, y0: t.target_price, y1: t.target_price, line: { color: rgbaAlta(0.7), width: 1, dash: 'dot' } })
      }
    }
  }
