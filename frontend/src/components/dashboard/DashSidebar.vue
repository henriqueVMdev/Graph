<template>
  <aside class="bg-surface-800 border-r border-surface-500 flex flex-col h-full overflow-y-auto">
    <div class="p-4 space-y-5">
      ...
      <!-- Info da Otimizacao -->
      <div v-if="store.hasData && (store.asset || store.timeframe)">
        <h2 class="sidebar-section-title">Info da Otimizacao</h2>
        ...
        <div v-if="Object.keys(store.gridSummary).length > 0" class="mt-3">
          ...
          <div class="space-y-1.5">
            <div
              ...
              class="text-xs"
            >
              <span class="text-gray-400">{{ param }}:</span>
              <span class="ml-1 text-gray-300">
                <template v-if="values.length <= 5">{{ values.join(', ') }}</template>
                <template v-else>{{ values[0] }} a {{ values[values.length - 1] }} ({{ values.length }} valores)</template>
              </span>
            </div>
          </div>
        </div>
